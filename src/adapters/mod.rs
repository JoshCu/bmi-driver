//! BMI adapter implementations.
//!
//! Each adapter wraps a different language runtime and translates the Rust [`Bmi`] trait into the
//! corresponding foreign calls. The table below summarises which standard BMI functions each
//! adapter actually invokes on the underlying model. Functions marked with a note deviate from the
//! straightforward name-match.
//!
...
//! | `get_var_units` | ✓ | ✓ | ✓ | ✓ |
//! | `get_var_itemsize` | ✓ | ✓ | ✓ | ✓ |
//! | `get_var_nbytes` | ✓ (used to size get_value buffer) | ✓ (used to size get_value buffer) | — (Python manages memory) | ✓ |
//! | `get_var_location` | ✓ | ✓ | ✓ | ✓ |
//! | `get_current_time` | ✓ | ✓ | ✓ | ✓ |
...
//!
//! The [`crate::ffi::Bmi`] C struct exposes additional optional function pointers that are
//! defined in the full BMI specification but are not wrapped by the Rust [`Bmi`] trait and are
//! therefore never called by the driver:
//!
...
//!
//! [`BmiC`]: crate::adapters::BmiC
//! [`BmiFortran`]: crate::adapters::BmiFortran
//! [`BmiPython`]: crate::adapters::BmiPython
//! [`BmiSloth`]: crate::adapters::BmiSloth
//! [`Bmi`]: crate::traits::Bmi
