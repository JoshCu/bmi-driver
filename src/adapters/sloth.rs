...
impl BmiSloth {
    ...
    pub fn configure(&mut self, params: &HashMap<String, String>) -> BmiResult<()> {
        ...
        let cache: HashMap<String, VarType> = self.variables.iter()
    ...
    fn parse_param(key: &str, val: &str) -> BmiResult<Option<(String, SlothVar)>> {
        ...
        Ok(Some((name, SlothVar { count, var_type, type_str, units, location, value })))
    }

    fn get_var(&self, name: &str) -> BmiResult<&SlothVar> {
        self.variables.get(name).ok_or_else(|| BmiError::FunctionFailed {
...
impl Bmi for BmiSloth {
    ...
    fn get_component_name(&self) -> BmiResult<String> { Ok(format!("SLOTH ({})", self.name)) }
    fn get_input_item_count(&self) -> BmiResult<i32> { Ok(0) }
    fn get_output_item_count(&self) -> BmiResult<i32> { Ok(self.var_names.len() as i32) }
    fn get_input_var_names(&self) -> BmiResult<Vec<String>> { Ok(vec![]) }
    fn get_output_var_names(&self) -> BmiResult<Vec<String>> { Ok(self.var_names.clone()) }

    fn get_var_grid(&self, _name: &str) -> BmiResult<i32> { Ok(0) }
    ...
    fn get_current_time(&self) -> BmiResult<f64> { Ok(self.current_time) }
    fn get_start_time(&self) -> BmiResult<f64> { Ok(0.0) }
    fn get_end_time(&self) -> BmiResult<f64> { Ok(f64::MAX) }
    fn get_time_units(&self) -> BmiResult<String> { Ok("s".into()) }
    fn get_time_step(&self) -> BmiResult<f64> { Ok(self.time_step) }
    ...
    fn set_value_f64(&mut self, name: &str, values: &[f64]) -> BmiResult<()> {
        if let Some(var) = self.variables.get_mut(name) {
            if let Some(&v) = values.first() { var.value = v; }
        }
        Ok(())
    }
    ...
    fn set_value_f32(&mut self, name: &str, values: &[f32]) -> BmiResult<()> {
        if let Some(var) = self.variables.get_mut(name) {
            if let Some(&v) = values.first() { var.value = v as f64; }
        }
        Ok(())
    }
    ...
    fn get_grid_rank(&self, _grid: i32) -> BmiResult<i32> { Ok(1) }
    fn get_grid_size(&self, _grid: i32) -> BmiResult<i32> { Ok(1) }
    fn get_grid_type(&self, _grid: i32) -> BmiResult<String> { Ok("scalar".into()) }
}
