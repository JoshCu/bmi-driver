...
impl NetCdfForcings {
    ...
    pub fn preload_location(&mut self, loc_id: &str) -> BmiResult<()> {
        if self.cached_loc.as_deref() == Some(loc_id) {
            return Ok(());
        }
        ...
        let file = self
            .file
            .as_ref()
            .ok_or_else(|| self.err("not initialized"))?;

        self.cached_data.clear();
        for var_name in &self.var_names {
            let var = file
                .variable(var_name)
                .ok_or_else(|| self.err(&format!("Variable '{}' not found", var_name)))?;
    ...
    fn load_metadata(&mut self) -> BmiResult<()> {
        ...
        let ids_var = file
            .variable("ids")
            .ok_or_else(|| self.err("Missing 'ids' variable"))?;
        ...
        for i in 0..ids_var.len() {
            let id = ids_var
                .get_string(i)
                .map_err(|e| self.err(&format!("Failed to get id {}: {}", i, e)))?;
        ...
        self.timestep_count = file
            .dimension("time")
            .map(|d| d.len())
            .ok_or_else(|| self.err("Missing 'time' dimension"))?;
    ...
    fn load_time_info(&mut self) -> BmiResult<()> {
        ...
        if let Some(time_var) = file.variable("Time") {
            ...
            let times: Vec<i64> = if dims.len() == 2 {
                time_var
                    .get_values((0usize, ..))
                    .map_err(|e| self.err(&format!("Time read error: {}", e)))?
            } else {
                time_var
                    .get_values(..)
                    .map_err(|e| self.err(&format!("Time read error: {}", e)))?
    ...
    fn discover_vars(&mut self) -> BmiResult<()> {
        ...
        for var in file.variables() {
            ...
            if !dims.contains(&"catchment-id".to_string()) || !dims.contains(&"time".to_string()) {
                continue;
            }
...
impl Forcings for NetCdfForcings {
    ...
    fn initialize(&mut self, path: &str) -> BmiResult<()> {
        ...
        if !Path::new(path).exists() {
            return Err(BmiError::ConfigNotFound { path: path.into() });
        }

        self.file =
            Some(netcdf::open(path).map_err(|e| self.err(&format!("Failed to open: {}", e)))?);
    ...
    fn var_type(&self, name: &str) -> BmiResult<String> {
        self.var_info
            .get(name)
            .map(|i| i.type_str.clone())
            .ok_or_else(|| self.err(&format!("Unknown var: {}", name)))
    }
    ...
    fn get_f32(&self, name: &str, loc: &str, step: usize) -> BmiResult<f32> {
        if let Some(data) = self
            ...
            .as_ref()
            .filter(|l| *l == loc)
            .and_then(|_| self.cached_data.get(name))
        {
            return data
                .get(step)
                .copied()
        ...
        let var = file
            .variable(name)
            .ok_or_else(|| self.err(&format!("Variable '{}' not found", name)))?;
        let vals: Vec<f32> = var
            .get_values((loc_idx, step..step + 1))
            .map_err(|e| self.err(&format!("Read error: {}", e)))?;
        vals.into_iter()
            .next()
            .ok_or_else(|| self.err("Empty result"))
    }
