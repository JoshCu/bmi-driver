...
fn err(msg: String) -> BmiError {
    BmiError::FunctionFailed { model: "netcdf_writer".into(), func: msg }
}
...
impl NetCdfWriter {
    pub fn new(
        ...
        let handle = thread::spawn(move || {
            writer_thread(rx, path, &start_time, interval, total_steps)
        });
    ...
    pub fn write(&self, result: LocationResult) -> BmiResult<()> {
        self.sender.send(WriterMessage::Write(result))
        ...
        match receiver.recv_timeout(Duration::from_millis(100)) {
            Ok(WriterMessage::Write(result)) => {
                ...
                if batch.len() >= batch_size {
                    ...
                    location_count = write_batch(file.as_mut().unwrap(), &batch, &var_names, location_count)?;
                    batch.clear();
                }
            }
...
fn init_netcdf(
    ...
    time_var.put_attribute("units", format!("seconds since {}", start_time))
    ...
    let var_names: Vec<String> = first_result.columns.iter().map(|(name, _)| name.clone()).collect();
    for name in &var_names {
        let mut var = file.add_variable::<f32>(name, &["catchment_id", "time"])
...
fn write_batch(
    ...
    let batch_maps: Vec<HashMap<&str, &Vec<f64>>> = batch.iter().map(|r| {
        r.columns.iter().map(|(name, vals)| (name.as_str(), vals)).collect()
    }).collect();

    // Write location IDs
    let mut ids_var = file.variable_mut("ids")
        .ok_or_else(|| err("ids variable not found".into()))?;
    for (i, result) in batch.iter().enumerate() {
        ids_var.put_string(&result.location_id, start_idx + i)
            .map_err(|e| err(format!("Failed to write id '{}': {}", result.location_id, e)))?;
    }

    // Write each output variable
    for var_name in var_names {
        let mut var = file.variable_mut(var_name)
        ...
        for (i, batch_map) in batch_maps.iter().enumerate() {
            if let Some(vals) = batch_map.get(var_name.as_str()) {
                ...
                var.put_values(&f32_vals, (start_idx + i, ..))
                    .map_err(|e| err(format!("Failed to write '{}' for batch item {}: {}", var_name, i, e)))?;
            }
        }
    }
...
pub fn merge_netcdf_files(worker_files: &[PathBuf], output_path: &Path) -> BmiResult<()> {
    ...
    if worker_files.len() == 1 {
        std::fs::rename(&worker_files[0], output_path)
            .map_err(|e| err(format!("Failed to rename {} to {}: {}", worker_files[0].display(), output_path.display(), e)))?;
        return Ok(());
    }

    // Open first file to get structure
    let first = netcdf::open(&worker_files[0])
        .map_err(|e| err(format!("Failed to open {}: {}", worker_files[0].display(), e)))?;

    let n_times = first.dimension("time")
        .ok_or_else(|| err("time dimension not found in worker file".into()))?
        .len();

    // Read time values and attributes
    let time_var = first.variable("time")
        .ok_or_else(|| err("time variable not found".into()))?;
    let time_values: Vec<f64> = time_var.get_values(..)
        .map_err(|e| err(format!("Failed to read time values: {}", e)))?;
    let time_units: String = time_var.attribute("units")
    ...
    let var_names: Vec<String> = first.variables()
        .filter(|v| {
            let name = v.name();
            name != "time" && name != "ids"
        })
    ...
    merged.add_dimension("time", n_times)
        .map_err(|e| err(format!("Failed to add time dim: {}", e)))?;
    merged.add_unlimited_dimension("catchment_id")
        .map_err(|e| err(format!("Failed to add catchment_id dim: {}", e)))?;

    let mut time_out = merged.add_variable::<f64>("time", &["time"])
        .map_err(|e| err(format!("Failed to add time var: {}", e)))?;
    if !time_units.is_empty() {
        time_out.put_attribute("units", time_units)
            .map_err(|e| err(format!("Failed to set time units: {}", e)))?;
    }
    time_out.put_attribute("long_name", "valid output time")
        .map_err(|e| err(format!("Failed to set time long_name: {}", e)))?;
    time_out.put_values(&time_values, ..)
        .map_err(|e| err(format!("Failed to write time: {}", e)))?;

    merged.add_string_variable("ids", &["catchment_id"])
        .map_err(|e| err(format!("Failed to add ids var: {}", e)))?;

    for name in &var_names {
        let mut var = merged.add_variable::<f32>(name, &["catchment_id", "time"])
    ...
    for worker_path in worker_files {
        ...
        let n_locs = src.dimension("catchment_id")
            .map(|d| d.len())
            .unwrap_or(0);
        if n_locs == 0 { continue; }

        // Copy location IDs
        let ids_src = src.variable("ids")
            .ok_or_else(|| err("ids variable not found in worker file".into()))?;
        let mut ids_dst = merged.variable_mut("ids")
            .ok_or_else(|| err("ids variable not found in merged file".into()))?;
        for i in 0..n_locs {
            let id = ids_src.get_string(i)
                .map_err(|e| err(format!("Failed to read id at {}: {}", i, e)))?;
            ids_dst.put_string(&id, out_idx + i)
        ...
        for name in &var_names {
            if let Some(src_var) = src.variable(name) {
                let mut dst_var = merged.variable_mut(name)
                    .ok_or_else(|| err(format!("'{}' not found in merged file", name)))?;
                for i in 0..n_locs {
                    let row: Vec<f32> = src_var.get_values((i, ..))
                        .map_err(|e| err(format!("Failed to read '{}' row {}: {}", name, i, e)))?;
                    dst_var.put_values(&row, (out_idx + i, ..))
                        .map_err(|e| err(format!("Failed to write '{}' row {}: {}", name, out_idx + i, e)))?;
                }
            }
        }
